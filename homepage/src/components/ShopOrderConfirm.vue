<template>
  <!-- 訂單確認 LightBox -->
  <div class="bg_shop_confirm">
    <div class="wrap_shop_confirm">
      <div class="shop_conf_price">
        <h4>應付金額：＄ {{ itemTotal }}</h4>
        <hr />
      </div>
      <div class="shop_conf_credit">
        <h4>信用卡資訊</h4>
        <p>信用卡卡號：{{ confirmation.text }}</p>
        <hr />
      </div>
      <div class="shop_conf_order_info">
        <h4>收件人資訊</h4>
        <p>收件人名字：{{ member.mem_name }}</p>
        <p>收件人電話：{{ member.mem_phone }}</p>
        <p>收件人縣市：{{ member.mem_city }}</p>
        <p>收件人地址：{{ member.mem_addr }}</p>
      </div>
      <div class="confirm_btn_box">
        <button class="btn_confirm" type="button" @click="orderCompleted">
          確認無誤
        </button>
        <button @click="cancel" class="btn_return" type="button">取消</button>
      </div>
    </div>
  </div>
  <!-- 訂單已成立 LightBox -->
  <div v-if="orderCompletedBox" class="container_shop_order_completed">
    <div class="wrap_order_completed">
      <div class="order_confirm_content">
        <h4>訂單已成立囉～</h4>
        <hr />
        <p>訂單狀態：已付款</p>
      </div>
      <div class="confirm_btn_box">
        <button class="btn_return" @click="close" type="button">關閉</button>
      </div>
    </div>
  </div>
</template>
<script>
export default {
  props: ['confirmation', 'member', 'itemTotal'],
  data() {
    return {
      orderListDetail: [],
      orderCompletedBox: false,
    };
  },
  methods: {
    cancel() {
      this.$emit('cancelBox', this.confirmBox);
    },
    close() {
      this.$router.push('/Shopping');
    },
    orderCompleted() {
      this.orderSend();
    },
    getOrderList() {
      this.orderListDetail = localStorage.getItem('cart');
      if (!this.orderListDetail || this.orderListDetail === 'undefined') return;
      this.orderListDetail = JSON.parse(this.orderListDetail);
    },
    orderSend() {
      let xhr = new XMLHttpRequest();
      xhr.open(
        'POST',
        process.env.VUE_APP_PHP_PATH + 'product_order.php',
        true
      );

      let formData = new FormData();
      formData.append('mem_no', this.member.mem_no);
      formData.append('mem_city', this.member.mem_city);
      formData.append('mem_addr', this.member.mem_addr);
      formData.append('order_payment', this.itemTotal);
      xhr.send(formData);
      xhr.onload = () => {
        if (xhr.status == 200) {
          this.orderListSend(xhr.response);
        }
      };
      xhr.onerror = (err) => {
        console.log(err);
      };
    },

    orderListSend(getOrderListNo) {
      for (let i = 0; i < this.orderListDetail.length; i++) {
        let xhr = new XMLHttpRequest();
        xhr.open(
          'POST',
          process.env.VUE_APP_PHP_PATH + 'product_order_list.php',
          true
        );

        let formData = new FormData();
        formData.append('product_order_no', getOrderListNo);
        formData.append('product_no', this.orderListDetail[i].product_no);
        formData.append(
          'product_order_list_qty',
          this.orderListDetail[i].product_qty
        );
        formData.append(
          'product_order_list_price',
          this.orderListDetail[i].product_price
        );
        xhr.send(formData);
        xhr.onload = () => {
          if (xhr.status == 200) {
            this.orderCompletedBox = true;
          }
        };
        xhr.onerror = (err) => {
          console.log(err);
        };
      }
    },
  },
  created() {
    this.getOrderList();
  },
};
</script>

<style lang="scss" scoped>
@import '../assets/scss/style.scss';

.bg_shop_confirm,
.container_shop_order_completed {
  position: fixed;
  z-index: 1000;
  top: 0;
  left: 0;
  margin: auto;
  width: 100%;
  height: 100%;
  background-color: rgba(0, 0, 0, 0.5);
  transition: opacity 0.3s ease;
}
.wrap_shop_confirm,
.wrap_order_completed {
  box-sizing: border-box;
  position: absolute;
  top: 50%;
  left: 50%;
  transform: translate(-50%, -50%);
  padding: 1rem;
  width: 350px;
  height: auto;
  background-color: #fff;
  border-radius: 10px;
  > div,
  p,
  h4 {
    padding: 0.5rem;
  }
}
.confirm_btn_box {
  display: flex;
  justify-content: center;
  > button {
    margin: 0.5rem;
  }
}
</style>
